...
//
// Notably, CAPI controllers do not trigger a deletion of the RKEBootstrap object if a pre-terminate annotation exists on it --
// this means we rely on the OnChange handler to perform node safe removal annotation. When run from OnRemove, it is expected that the
